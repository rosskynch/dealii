...
//TODO: Maybe we should figure out if the element really needs this
  ...
  namespace internals
  {
                                                ...
                                                std::vector<bool> &dofs_processed)
    {
      ...
      {
        case 2:
          // This is very similar to compute_edge_projection as used in 3D
        {
          ...
          if (associated_edge_dofs != degree + 1)
          {
            std::cout << "2D edge DoFs found(" << associated_edge_dofs
            << ") != degree+1(" << degree + 1
            << ")" << std::endl;
          }
        ...
        case 3:
        {
          ...
          std::vector<std::vector<unsigned int>> associated_edge_dof_to_face_dof(lines_per_face, std::vector<unsigned int> (degree + 1));
          ...
          /* Next find the face DoFs are associated with the vector components
          ...
          // Sanity check:
          unsigned int associated_face_dofs = associated_face_dof_index;
          if (associated_face_dofs != 2*degree*(degree + 1))
          {
            std::cout << "DoFs found(" << associated_face_dofs
            << ") != 2*degree*(degree+1)(" << 2*degree*(degree + 1)
            << ")" << std::endl;
          }
          ...
          // Store normal all vectors at quad points:
          ...
          for (unsigned int q_point = 0;
               q_point < fe_face_values.n_quadrature_points; ++q_point)
               {
                  ...
                  * 
                  * The RHS entries are:
                  * \int_{face} (n x (Edge_residual) \cdot (n x face_shape_function_i)
                  */
                                           ...
                                           const Mapping<dim> &mapping)
  {
     ...
     * 
     * In 3D we need to constrain edge and face DoFs.
     *
     * For edges the tangential component is interpolated on edge.
     * This returns the values for the DoFs associated with each edge shape
     * function. 
     * 
    ...
    const QGauss<dim - 1> reference_face_quadrature (2 * superdegree + 3);
    ...
    switch (dim)
    {
      ...
      case 3:
      {
        const QGauss<dim-2> reference_edge_quadrature (2 * superdegree + 3);
