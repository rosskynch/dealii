...
//TODO: Maybe we should figure out if the element really needs this
  ...
  namespace internals
  {
                             ...
                             std::vector<bool> &dofs_processed)
    {
       ...
       * Want to know which of these DoFs (there are degree+1 of them)
       * are associated with the components given by first_vector_component.
       * Then we can make a map from the required line DoFs to the face DoFs.
       * 
       * For a single FE_Nedelec element this is simple:
...
//         unsigned int face_idx = line*fe.dofs_per_line + line_idx;
        ...
        std::cout << " face " << face_idx << " local dof " << cell_idx << std::endl;
...
//                            * GeometryInfo<dim>::faces_per_cell + face);
      ...
      if (dynamic_cast<const FESystem<dim>*> (&cell->get_fe ()) != 0)
      {
         ...
         * base_indices.first
         *
         * Then select which copy of that base elements
         * [ each copy is of length fe.base_element(base_indices.first).n_components() ]
         * corresponds to first_vector_component:
         * base_index.second
         */
...
//              unsigned int face_idx = line*fe.dofs_per_line + line_idx;
          ...
          unsigned int lower_cell_bound = fe.base_element (base_indices.first)
                            .face_to_cell_index(lines_per_face * (fe.degree+1), face);
          ...
          // Sanity check:
          unsigned int num_component_face_dofs=face_index;
          if (num_component_face_dofs != 2*degree*(degree+1))
          {
            std::cout << "DoFs found(" << num_component_face_dofs
            << ") != 2*fe.degree*(fe.degree+1)(" << 2*fe.degree*(fe.degree+1)
            << ")" << std::endl;
          }
...
//         {
          for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
          {
            for (unsigned int line = 0; line < GeometryInfo<dim>::lines_per_face; ++line)
            {
              ...
              /*
              Point<dim> p1 = cell->face (face)->line (line)->vertex(0);
              Point<dim> p2 = cell->face (face)->line (line)->vertex(1);
              edge_quadrature_collection.push_back
              (QProjector<dim>::project_to_line(reference_edge_quadrature,p1,p2));             
              */
            }
          }
//         }
